...
router.post("/signup", async (req, res, next) => {
  ...
  try {
    ...
    const { email:emailUser, name:nameUser, _id } = createdUser;

    // Create a new object that doesn't expose the password
    const user = { email:emailUser, name:nameUser, _id };
...
router.get("/logout", isAuthenticated, async(req,res, next)=>{
  try{
    ...
    res.status(403).json({message: "Error, coul not logout, try again"})
  }
})
